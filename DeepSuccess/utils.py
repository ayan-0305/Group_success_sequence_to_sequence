# This file contains the code for defining the loss functions as well as training and testing of the model. It also defines functions for computing the Accuracy for classification as well as the Binning Accuracy for estimating success metrics 
...
def trainItersCombined(train_list, train_list2, metadata, metadata2, model_name, hidden_size, encoder, decoder,
                 encoder2, decoder2, fmodel, encoder_optimizer, decoder_optimizer,
                 encoder_optimizer2, decoder_optimizer2, fmodel_optimizer,
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        loss3, ploss3 = trainf(fmodel, inp_vec, other, device)
        ...
        if iteration % print_every == 0:
            ...
            print("Iteration: {}; Tf_ration: {:.4f}; Percent complete: {:.1f}%; Average loss: {:.6f}".format(iteration, tf_ratio, iteration / n_iteration * 100, print_loss_avg))
            print_loss = 0
...
def pcAccuracy(fmodel, f_out, f_out2, other, device, thres=0.5):
    inp_vec = valtovec(f_out, f_out2, other, device)
