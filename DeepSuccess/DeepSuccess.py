# This is the main file to generate the results of DeepSuccess model as reported in the paper.
...
from model import EncoderRNN, LuongAttnDecoderRNN, DecoderRNN, FinalModel
from utils import trainItersCombined, eval_data, pAccuracy, pAccuracy_bin, pcAccuracy

USE_CUDA = torch.cuda.is_available()
device = torch.device("cuda" if USE_CUDA else "cpu")
MAX_INP_LEN = 5 # Number of consecutive events in the encoder 
MAX_LENGTH = 10 # Total number of consecutive events for a group, first 5 for the encoder while last 5 is for the decoder
...
train_list = data[:trr]
test_list = data[trr:]
print("size of train data 1: ", len(train_list))
# print("some sample ======================")
# print(train_list[:2])
...
# Configure models using the following parameter values
model_name = 's2s_model_pytorch'
attn_model = 'dot' # Scoring function for attention model
#attn_model = 'general'
#attn_model = 'concat'
input_dim = 20 # Dimension of feature vectors for individual events of a group
output_dim = 1
hidden_size = 50 # Hidden dimension of LSTMs
encoder_n_layers = 2
decoder_n_layers = 2
dropout = 0.1 # Dropout
...
n_iteration = 2000 # No. of epochs used for training
print_every = 1
save_every = 500
...
print('Building optimizers ...') # Use the Adam optimization 
...
trainItersCombined(train_list, train_list2, metadata, metadata2, model_name, hidden_size, 
            encoder, decoder, encoder2, decoder2, fmodel, encoder_optimizer,
            decoder_optimizer, encoder_optimizer2, decoder_optimizer2, fmodel_optimizer,
...
pcAccuracy(fmodel, f_out, f_out2, others, device, thres=0.5)
print("----> Group size")
pAccuracy(others, f_out, f_tar, metadata, 1)
pAccuracy_bin(f_out, f_tar, [4,5,10])
print("----> Event attendance")
pAccuracy(others2, f_out2, f_tar2, metadata2, 0)
pAccuracy_bin(f_out2, f_tar2, [4,5,10])
